...
task_allocate_threadid(task_system *ts)
{
   ...
   uint64 tid_array_index = 0;
   uint64   old_bitmask;
   uint64   new_bitmask;

   do {
      uint64 pos = 0;
      while (pos == 0 && tid_array_index < TID_ARRAY_SIZE) {
         old_bitmask = tid_bitmask[tid_array_index];
         // first bit set to 1 starting from LSB.
         pos = __builtin_ffsl(old_bitmask);
         tid_array_index = (pos == 0)? tid_array_index + 1 : tid_array_index;
      }
      ...
      tid = 64 * tid_array_index + pos - 1;
      // set bit at that position to 0, indicating in use.
      new_bitmask = (old_bitmask & ~(1ULL << (pos - 1)));
   } while (
      !__sync_bool_compare_and_swap(&tid_bitmask[tid_array_index], old_bitmask, new_bitmask));
...
{
   uint64 *tid_bitmask = task_system_get_tid_bitmask(ts);
   uint64 tid_array_index = tid / 64;
   uint64 pos = tid % 64;
   uint64 bitmask_val = tid_bitmask[tid_array_index];

   // Ensure that caller is only clearing for a thread that's in-use.
   platform_assert(!(bitmask_val & (1ULL << pos)),
                   "Thread [%lu] is expected to be in-use. Bitmap: 0x%lx",
                   tid,
                   bitmask_val);

   // set bit back to 1 to indicate a free slot.
   uint64 tmp_bitmask = tid_bitmask[tid_array_index];
   uint64 new_value   = tmp_bitmask | (1ULL << pos);
   while (!__sync_bool_compare_and_swap(&tid_bitmask[tid_array_index], tmp_bitmask, new_value)) {
      tmp_bitmask = tid_bitmask[tid_array_index];
      new_value   = tmp_bitmask | (1ULL << pos);
   }
}
...
task_system_destroy(platform_heap_id hid, task_system **ts_in)
{
   ...
   for (uint64 i = 0; i < TID_ARRAY_SIZE; i++) {
      if (ts->tid_bitmask[i] != ((uint64)-1)) {
         platform_error_log(
